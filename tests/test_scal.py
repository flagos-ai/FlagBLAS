...
def cublas_scal_reference(n, alpha, x, incx, y, incy):
...
def test_accuracy_scal_real(dtype, shape, alpha, incx, incy):
    ...
    cublas_scal_reference(n, alpha, ref_x, incx, ref_y, incy)

    if dtype == torch.float32:
        flag_blas.ops.sscal(n, alpha, x, incx, y, incy)
        torch.testing.assert_close(y, ref_y, rtol=1e-5, atol=1e-5)
    else:
        flag_blas.ops.dscal(n, alpha, x, incx, y, incy)
