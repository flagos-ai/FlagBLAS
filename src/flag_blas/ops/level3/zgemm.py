...
def _select_zgemm_config(transa: int, transb: int, m: int, n: int, k: int):
    max_dim = max(m, n, k)
    if max_dim <= 32:
        return 16, 16, 16, 4, 1
    if max_dim <= 128:
...
def zgemm(
    ...
    block_m, block_n, block_k, num_warps, group_m = _select_zgemm_config(
        transa, transb, m, n, k
    )
