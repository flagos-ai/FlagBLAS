...
def abs_kernel_complex(
    ...
    # torch.view_as_real(x) 之后，最后一维长度为 2: [real, imag]
...
def _abs_impl(
    ...
    with torch_device_fn.device(x.device):
        if is_complex:
            ...
            abs_kernel_complex[grid](
                x_real, y, n, incx, incy, BLOCK_SIZE=block_size
            )
        else:
            abs_kernel_real[grid](
                x, y, n, incx, incy, BLOCK_SIZE=block_size
            )


def sabs(n: int, x: torch.Tensor, incx: int, y: torch.Tensor, incy: int) -> None:
    logger.debug("FLAG_BLAS SABS")
    assert x.dtype == torch.float32, "x must be float32"
    assert y.dtype == torch.float32, "y must be float32"
    _abs_impl(n, x, incx, y, incy, is_complex=False)


def dabs(n: int, x: torch.Tensor, incx: int, y: torch.Tensor, incy: int) -> None:
...
def cabs(n: int, x: torch.Tensor, incx: int, y: torch.Tensor, incy: int) -> None:
    logger.debug("FLAG_BLAS CABS")
    assert x.dtype == torch.complex64, "x must be complex64"
    assert y.dtype == torch.float32, "y must be float32"
    _abs_impl(n, x, incx, y, incy, is_complex=True)


def zabs(n: int, x: torch.Tensor, incx: int, y: torch.Tensor, incy: int) -> None:
