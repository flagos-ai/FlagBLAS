...
# Autotune configs
#
# stbsv is a sequential triangular solve: row j depends on previously
# solved rows. We therefore launch a *single* program (grid=(1,)) that
# scans the n unknowns in order, vectorising only the trailing band
# update (BLOCK_K wide).  num_warps=1 is used so that within-program
# stores are guaranteed to be visible to subsequent loads without any
# extra memory fence.
# --------------------------------------------------------------------------
_STBSV_CONFIGS = [
    triton.Config({"BLOCK_K": 32}, num_warps=1, num_stages=1),
    triton.Config({"BLOCK_K": 64}, num_warps=1, num_stages=1),
    triton.Config({"BLOCK_K": 64}, num_warps=1, num_stages=2),
    triton.Config({"BLOCK_K": 128}, num_warps=1, num_stages=1),
    triton.Config({"BLOCK_K": 128}, num_warps=1, num_stages=2),
    triton.Config({"BLOCK_K": 256}, num_warps=1, num_stages=1),
]
...
# Kernel
# --------------------------------------------------------------------------
@triton.autotune(configs=_STBSV_CONFIGS, key=_TBSV_KEY, restore_value=_TBSV_RESTORE)
@triton.jit
def stbsv_kernel(
    a_ptr, x_ptr,
    n, k, LDA, INCX,
    ...
    # ----------------------------------------------------------------------
    # Lower / NoTrans : solve  L x = b   (forward substitution)
    #   for j = 0 .. n-1:
    #       x[j] /= A[0, j]                     if not UNIT
    #       for d = 1 .. k, j+d < n:
    #           x[j+d] -= A[d, j] * x[j]
    # ----------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    # Upper / NoTrans : solve  U x = b   (back substitution)
    #   for j = n-1 .. 0:
    #       x[j] /= A[k, j]                     if not UNIT
    #       for d = 1 .. k, j-d >= 0:
    #           x[j-d] -= A[k-d, j] * x[j]
    # ----------------------------------------------------------------------
    ...
    #   for j = 0 .. n-1:
    #       s = sum_{d=1..k, j-d>=0} A[k-d, j] * x[j-d]
    #       x[j] = (x[j] - s) / A[k, j]        if not UNIT
    # ----------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    # Lower / Trans   : solve  L^T x = b   (back substitution)
    #   for j = n-1 .. 0:
    #       s = sum_{d=1..k, j+d<n} A[d, j] * x[j+d]
    #       x[j] = (x[j] - s) / A[0, j]        if not UNIT
    # ----------------------------------------------------------------------
    else:
        for jc in tl.range(0, n):
            j = n - 1 - jc
...
def stbsv(
    ...
    """Solve a real single-precision triangular banded system in-place.
        ...
        op(A) * x = b,   op(A) in {A, A^T}

    where A is n-by-n triangular banded with k sub/super-diagonals stored
    in cuBLAS banded format (leading (k+1)-by-n column-major).  The
    right-hand side b is passed in via ``x`` and overwritten by the
    solution in place.
    """
