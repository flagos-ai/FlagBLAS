...
GROUP_BFGEMM_LLC_OPTIONS = (
    "-mtgpu-enable-preisel-sinking=1 "
    "-mtgpu-enable-int-hidden=1 "
    "-mtgpu-use-sl=1 "
    "-mtgpu-enable-postra-sched=1 "
    "-mtgpu-reorder-scheduling=1"
)
GROUP_BFGEMM_TLE_LLC_OPTIONS = (
    GROUP_BFGEMM_LLC_OPTIONS
    + " -mtgpu-sqmma-sl-opt=1 -mtgpu-high-latency-instrs-lifting-as-possible=1"
)
...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("group_bfgemm"), key=["M", "N", "K", "group_size"]
)
@triton.jit
        ...
        if tile_idx >= last_problem_end and tile_idx < current_problem_end:
            ...
            for _ in tl.range(loop_count):
                ...
                tile_m_idx, tile_n_idx = grouped_launch(
                    group_tile_idx, m, N, BLOCK_M, BLOCK_N, GROUP_M
                )
                ...
                c_ptrs = (
                    group_out
                    + offs_am.to(tl.int64) * N
                    + offs_bn
                    + N * tl.arange(0, BLOCK_M)[:, None]
...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("group_bfgemm_m256"),
    key=["M", "N", "K", "group_size"],
)
@triton.jit
def grouped_bfgemm_tma_m256_kernel(
    M,
    N: tl.constexpr,
    K: tl.constexpr,
    a_desc,
        ...
        if tile_idx >= last_problem_end and tile_idx < current_problem_end:
            ...
            for _ in tl.range(loop_count):
                ...
                for k in tl.range(0, tl.cdiv(K, block_k)):
                    ...
                    accumulator_lo = tl.dot(
                        a_lo, b, acc=accumulator_lo, out_dtype=tl.float32
                    )
                    accumulator_hi = tl.dot(
                        a_hi, b, acc=accumulator_hi, out_dtype=tl.float32
                    )
                ...
                tl.store(
                    c_ptrs,
                    accumulator_lo.to(tl.bfloat16),
                    mask=(offs_am + offs_m[:, None] < group_end)
                    & (offs_bn + offs_n[None, :] < N),
                )
                tl.store(
                    c_ptrs + 128 * N,
                    accumulator_hi.to(tl.bfloat16),
                    mask=(offs_am + 128 + offs_m[:, None] < group_end)
                    & (offs_bn + offs_n[None, :] < N),
                )
                tile_idx += total_grid
        last_problem_end = current_problem_end
...
def matmul_tle_ws_set_block_size_hook(nargs):
    block_m = nargs["BLOCK_M"]
    block_n = nargs["BLOCK_N"]
    block_k = nargs["BLOCK_K"]
    nargs["a_desc"].block_shape = [block_m, block_k]
...
def prune_tle_ws_configs(configs, named_args, **kwargs):
    """Avoid known MThreads TLE resource failures before benchmarking."""
    if named_args["N"] == 384:
        valid_pairs = {(2, 5)}
    elif named_args["K"] == 384:
        valid_pairs = {(4, 3), (8, 3)}
    elif named_args["K"] == 192:
        valid_pairs = {(16, 6)} if named_args["N"] >= 4096 else {(8, 6)}
    elif named_args["K"] == 2048 and named_args["N"] == 768:
        valid_pairs = {(3, 4)}
    elif (
        named_args["K"] == 2048
        and named_args["N"] == 1536
        and (named_args["group_size"] >= 64 or named_args["group_size"] == 16)
    ):
        valid_pairs = {(6, 4)}
    elif (
        named_args["group_size"] <= 17
        and named_args["K"] == 4096
        and named_args["N"] >= 3072
    ):
        valid_pairs = {(8, 4)}
    elif named_args["group_size"] <= 17 and named_args["N"] >= 4096:
        valid_pairs = {(4, 4)}
    else:
        return [
            config
            for config in configs
            if config.kwargs.get("PIPELINE_STAGES") in {3, 4}
        ]
    return [
        config
        for config in configs
        if (config.kwargs.get("PANEL_WIDTH"), config.kwargs.get("PIPELINE_STAGES"))
        in valid_pairs
    ]


for config in runtime.get_tuned_config("group_bfgemm_tle_ws"):
    config.pre_hook = matmul_tle_ws_set_block_size_hook
    config.kwargs["enable_backend_opt"] = True
    config.kwargs["llc_options"] = GROUP_BFGEMM_TLE_LLC_OPTIONS


@triton.jit
def grouped_bfgemm_tma_m256_k64_kernel(
    M,
    N: tl.constexpr,
    K: tl.constexpr,
    a_desc,
    ...
    for group_idx in tl.range(group_size):
        ...
        if tile_idx >= last_problem_end and tile_idx < current_problem_end:
            ...
            for _ in tl.range(loop_count):
                group_tile_idx = tile_idx - last_problem_end
                tile_m_idx, tile_n_idx = grouped_launch(
                    group_tile_idx, m, N, block_m, 256, 1
                )
                offs_am = group_start + tile_m_idx * block_m
...
@triton.jit
def grouped_bfgemm_tma_schedule_m256_kernel(
    M,
    N: tl.constexpr,
    K: tl.constexpr,
    a_desc,
...
@triton.jit
def grouped_bfgemm_tma_schedule_m256_n128_kernel(
    M,
    N: tl.constexpr,
    K: tl.constexpr,
    a_desc,
...
@triton.jit
def grouped_bfgemm_tma_expert_m256_kernel(
    M,
    N: tl.constexpr,
    K: tl.constexpr,
    a_desc,
    ...
    if expert_idx < group_size:
        ...
        group_start = tl.load(
            group_list + expert_idx - 1, mask=expert_idx > 0, other=0
        ).to(tl.int32)
        ...
        for tile_m_idx in tl.range(tile_m_start, num_m_tiles, tiles_per_expert):
            ...
            c_ptrs = (
                group_out
                + (offs_am + offs_m[:, None]).to(tl.int64) * N
                + offs_bn
                + offs_n[None, :]
            )
            tl.store(
...
@triton.jit
def grouped_bfgemm_schedule_offsets_kernel(
    group_list,
    group_block_offsets,
    block_chunk_counts,
    group_size: tl.constexpr,
    ...
    group_start = tl.load(
        group_list + expert_idx - 1, mask=valid & (expert_idx > 0), other=0
    ).to(tl.int32)
...
@triton.jit
def grouped_bfgemm_schedule_chunk_offsets_kernel(
    block_chunk_counts,
    block_chunk_offsets,
    total_blocks,
    ...
    chunk_count = tl.load(block_chunk_counts + chunk_idx, mask=valid, other=0).to(
        tl.int32
    )
...
@triton.jit
def grouped_bfgemm_schedule_fill_kernel(
    group_list,
    group_block_offsets,
    block_chunk_offsets,
    b_row_starts,
    ...
    group_end = tl.load(group_list + expert_idx).to(tl.int32)
    group_start = tl.load(group_list + expert_idx - 1, mask=expert_idx > 0, other=0).to(
        tl.int32
    )
    block_count = tl.cdiv(group_end - group_start, BLOCK_M)
    block_offset = tl.load(group_block_offsets + expert_idx).to(tl.int32) + tl.load(
        block_chunk_offsets + expert_idx // BLOCK_E
    ).to(tl.int32)
...
@triton.jit
def grouped_bfgemm_pack_a_kernel(
    group_A,
    packed_A,
    c_row_starts,
    total_blocks,
    ...
    if schedule_idx < num_blocks:
        ...
        values = tl.load(
            group_A + (source_row + offs_m[:, None]) * K + offs_k[None, :],
            mask=(source_row + offs_m[:, None] < M) & (offs_k[None, :] < K),
            other=0.0,
        )
        tl.store(
            packed_A + (schedule_idx * BLOCK_M + offs_m[:, None]) * K + offs_k[None, :],
            values,
            mask=offs_k[None, :] < K,
        )
...
@triton.jit
def _grouped_bfgemm_rasterization_2d_column(
    block_idx, grid_x: tl.constexpr, grid_y, PANEL_WIDTH: tl.constexpr
):
    ...
    if grid_x % PANEL_WIDTH == 0:
        ...
        width = tl.where(
            block_idx >= full_panels_size, residual_panel_width, PANEL_WIDTH
        )
...
@triton.jit
def _grouped_bfgemm_tle_ws_consumer(
    reader,
    valid,
    tile_m,
    tile_n,
    ...
    if valid:
        ...
        for k_iter in tl.range(0, k_tiles, num_stages=1, loop_unroll_factor=k_tiles):
            ...
            accumulator = tle.gpu.wgmma(wait.slot.a, wait.slot.b, accumulator)
        ...
        if N % BLOCK_N != 0:
            ...
            tl.store(
                c_ptrs, accumulator.to(tl.bfloat16), mask=offs_m[:, None] < group_end
            )


@triton.jit
def _grouped_bfgemm_tle_ws_producer(
    writer,
    valid,
    tile_m,
    tile_n,
    ...
    if valid:
        ...
        for k_iter in tl.range(0, k_tiles, num_stages=1, loop_unroll_factor=k_tiles):
            ...
            tle.gpu.copy(a_desc, slot.a, (BLOCK_M, BLOCK_K), (row_start, offset_k))
            tle.gpu.copy(
                b_desc,
                slot.b,
                (BLOCK_K, BLOCK_N),
                (b_row_start + offset_k, tile_n * BLOCK_N),
            )
...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("group_bfgemm_tle_ws"),
    key=["M", "N", "K", "group_size"],
    prune_configs_by={"early_config_prune": prune_tle_ws_configs},
)
@triton.jit
def grouped_bfgemm_tle_ws_kernel(
    M,
    N: tl.constexpr,
    K: tl.constexpr,
    a_desc,
    ...
    pipe = tle.pipe(
        ...
        scope="cta",
        name="group_bfgemm_ab",
        a=a_smem,
        b=b_smem,
    )
    num_n_blocks: tl.constexpr = tl.cdiv(N, BLOCK_N)
    block_idx = tl.program_id(axis=0)
    tile_n, tile_m = _grouped_bfgemm_rasterization_2d_column(
        block_idx, num_n_blocks, num_m_blocks, PANEL_WIDTH
    )
    tle.gpu.warp_specialize(
        [
            (
                _grouped_bfgemm_tle_ws_consumer,
                (
                    pipe.reader(),
                    True,
                    tile_m,
            ...
            (
                _grouped_bfgemm_tle_ws_producer,
                (
                    pipe.writer(),
                    True,
                    tile_m,
...
@triton.jit
def _grouped_bfgemm_tle_ws_n192_consumer(
    a_reader,
    b0_reader,
    b1_reader,
    tile_m,
    ...
    offs_m = row_start + tl.arange(0, BLOCK_M)
    c_ptrs_0 = (
        group_out + offs_m[:, None].to(tl.int64) * 192 + tl.arange(0, 128)[None, :]
    )
    c_ptrs_1 = (
        group_out + offs_m[:, None].to(tl.int64) * 192 + 128 + tl.arange(0, 64)[None, :]
    )
...
@triton.jit
def _grouped_bfgemm_tle_ws_n192_producer(
    a_writer,
    b0_writer,
    b1_writer,
    tile_m,
...
@triton.jit
def grouped_bfgemm_tle_ws_n192_kernel(
    M,
    K: tl.constexpr,
    a_desc,
    b0_desc,
    ...
    b0_smem = tle.gpu.alloc(
        (PIPELINE_STAGES, BLOCK_K, 128),
        dtype=tl.bfloat16,
        scope=tle.gpu.smem,
        nv_mma_shared_layout=True,
    )
    b1_smem = tle.gpu.alloc(
        (PIPELINE_STAGES, BLOCK_K, 64),
        dtype=tl.bfloat16,
        scope=tle.gpu.smem,
        nv_mma_shared_layout=True,
    )
    a_pipe = tle.pipe(
        capacity=PIPELINE_STAGES, scope="cta", name="group_bfgemm_n192_a", a=a_smem
    )
    b0_pipe = tle.pipe(
        capacity=PIPELINE_STAGES, scope="cta", name="group_bfgemm_n192_b0", b=b0_smem
    )
    b1_pipe = tle.pipe(
        capacity=PIPELINE_STAGES, scope="cta", name="group_bfgemm_n192_b1", b=b1_smem
    )
    tile_m = tl.program_id(axis=0)
    tle.gpu.warp_specialize(
        [
            (
                _grouped_bfgemm_tle_ws_n192_consumer,
                (
                    a_pipe.reader(),
                    b0_pipe.reader(),
                    b1_pipe.reader(),
            ...
            (
                _grouped_bfgemm_tle_ws_n192_producer,
                (
                    a_pipe.writer(),
                    b0_pipe.writer(),
                    b1_pipe.writer(),
...
def grouped_hgemm_kernel(
    ...
    group_start = tl.load(group_list + group_idx - 1, mask=group_idx > 0, other=0).to(
        tl.int64
    )
...
def group_bfgemm(group_A, group_B, group_list, group_out):
    ...
    if K >= 64 and (N % 256 == 0 or N in (192, 384)) and 16 <= group_size <= 128:
        ...
        group_block_offsets = torch.empty(
            group_size, dtype=torch.int32, device=group_A.device
        )
        block_chunk_counts = torch.empty(
            num_chunks, dtype=torch.int32, device=group_A.device
        )
        block_chunk_offsets = torch.empty(
            num_chunks, dtype=torch.int32, device=group_A.device
        )
        ...
        packed_A = torch.empty(
            (max_blocks * block_m, K), dtype=group_A.dtype, device=group_A.device
        )
        grouped_bfgemm_schedule_offsets_kernel[(num_chunks,)](
            group_list,
            group_block_offsets,
            block_chunk_counts,
            group_size,
        ...
        grouped_bfgemm_schedule_chunk_offsets_kernel[(1,)](
            block_chunk_counts,
            block_chunk_offsets,
            total_blocks,
            num_chunks,
        ...
        grouped_bfgemm_schedule_fill_kernel[(group_size,)](
            group_list,
            group_block_offsets,
            block_chunk_offsets,
            b_row_starts,
        ...
        grouped_bfgemm_pack_a_kernel[(schedule_blocks, triton.cdiv(K, 64))](
            group_A,
            packed_A,
            c_row_starts,
            total_blocks,
        ...
        packed_a_desc = TensorDescriptor(
            packed_A, packed_A.shape, packed_A.stride(), dummy_block
        )
        ...
        if N == 192:
            ...
            grouped_bfgemm_tle_ws_n192_kernel[(schedule_blocks,)](
                M,
                K,
                packed_a_desc,
                b_desc,
            ...
            max_tiles = schedule_blocks * triton.cdiv(N, 256)
        ...
        if N == 128:
            ...
            grouped_bfgemm_tma_schedule_m256_n128_kernel[(num_aicores,)](
                M,
                N,
                K,
                a_desc,
                ...
                llc_options=GROUP_BFGEMM_LLC_OPTIONS,
            )
        else:
            b_desc.block_shape = [64, 256]
            grouped_bfgemm_tma_schedule_m256_kernel[(num_aicores,)](
                M,
                N,
                K,
                a_desc,
        ...
        grouped_bfgemm_tma_expert_m256_kernel[
            (group_size * tiles_per_expert * triton.cdiv(N, 256),)
        ](
            M,
            N,
            K,
        ...
        a_desc.block_shape = [128, 64]
        b_desc.block_shape = [64, 256]
        grouped_bfgemm_tma_m256_k64_kernel[(num_aicores,)](
            M,
            N,
            K,
            a_desc,
        ...
        grouped_bfgemm_tma_m256_kernel[(num_aicores,)](
            M, N, K, a_desc, b_desc, group_list, group_out, group_size
        )
    else:
