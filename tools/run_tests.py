...
# -*- coding: utf-8 -*-
"""
run_tests.py — Run accuracy and performance tests for FlagBLAS operators.
...
def _probe_flagblas():
    try:
        version = flag_blas.__version__
        repo = git.Repo(search_parent_directories=True)
        sha = repo.head.object.hexsha
        ver_str = f"{version}+git{sha[:8]}"
        ENV_INFO["flag_blas"] = {"version": ver_str}
        pinfo(f"flag_blas detected ... {ver_str}")
    except RuntimeError as e:
        perror(f"{e}")
        sys.exit(-1)
    except Exception as e:
        perror(f"{e}")
        perror("flag_blas has not been installed, please run `pip install -e .`")
        sys.exit(-1)

    try:
        vendor = flag_blas.vendor_name
        ENV_INFO["flag_blas"]["vendor"] = vendor
        pinfo(f"flag_blas vendor detection ... {vendor}")
    except Exception as e:
        perror(f"{e}")
        perror("flag_blas failed to detect vendor info.")
        sys.exit(-1)

    try:
        device = flag_blas.device
        ENV_INFO["flag_blas"]["device"] = device
        pinfo(f"flag_blas device detection ... {device}")
    except Exception as e:
        perror(f"{e}")
        perror("flag_blas failed to detect device info.")
        sys.exit(-1)
...
def parse_benchmark_log(log_file, op):
        ...
        with open(log_file, "r") as f:
            for line in f:
                ...
                try:
                    ...
                    # May be a list (attribute records) — skip these
                    continue
                # Only keep single objects with a "result" key (actual
    ...
    bench_res = {}
    for item in records:
        dtype = consts.DTYPE_MAP.get(item.get("dtype", ""), item.get("dtype", ""))
        details = {}
        total = 0.0
        count = 0
        for res in item.get("result", []):
            shape_detail = res.get("shape_detail", "Unknown")
            shape = str(shape_detail).replace(" ", "")
    ...
    return {
        "status": status,
        "data": bench_res,
        "test_case": records[0].get("op_name", op),
    }
...
def run_benchmark_q(gpu_id, op):
    ...
    Uses ``--record log --output benchmark_<op>.log`` so the result file has
    ...
    result_file = benchmark_dir / f"benchmark_{op}.log"
    if result_file.exists():
        try:
            result_file.unlink()
        except OSError:
    ...
    if not result_file.exists():
        status = "Error" if code != 0 else "NotFound"
        return {
            "status": status,
            "duration": dur,
            "exit_code": code,
    ...
    # Copy the log to the output directory for archival
    dest = op_dir / "performance_result.log"
    shutil.copy(str(result_file), str(dest))
    try:
        result_file.unlink()
    except OSError:
        pass
    ...
    record = {
        "duration": dur,
        "exit_code": code,
        "data_file": str(dest.relative_to(CFG.output_dir)),
        "data": {},
    }
...
def worker_proc(gpu_id, work_queue, display_queue):
    ...
    while True:
        ...
        customized_ops = [
            o[0] for o in flag_blas.runtime.backend.get_current_device_extend_op()
        ]
        result = {
