...
if IS_ASCEND:
    ...
    from flag_blas.runtime.backend._ascend.ops.group_gemm import grouped_hgemm_kernel
else:
    ...
    from flag_blas.runtime.backend._nvidia.hopper.ops.group_gemm import (
        ...
        grouped_hgemm_small_m_tma_kernel,
...
def gems_group_gemm_wrapper(
    ...
    **kwargs,
    ...
    if tma_available and kwargs.get("use_small_m", False):
        kernel = grouped_hgemm_small_m_tma_kernel
    elif tma_available:
        kernel = grouped_hgemm_tma_kernel
    else:
        kernel = grouped_hgemm_kernel
    ...
    def get_input_iter(self, cur_dtype) -> Generator:
        ...
        for k, e, n in self.shapes:
            ...
            for tensor in (group_A, group_B, group_list, out_aclnn):
                ...
                acl_dtype = ACL_INT64 if tensor.dtype == torch.int64 else ACL_FLOAT16
